...
from sqlalchemy import func, func, select, update, delete
...
async def get_all_products(
        ...
        created_at: datetime | None = Query(
            None, description="Дата создания товара (для фильтрации)"),
        updated_at: datetime | None = Query(
            None, description="Дата последнего обновления товара (для фильтрации)"),
    ...
    if created_at is not None:
        filters.append(func.date(ProductModel.created_at) >= created_at.date())
    if updated_at is not None:
        filters.append(func.date(ProductModel.updated_at) >= updated_at.date())
